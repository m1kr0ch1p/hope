"""
Serviço de sincronização de dados do CNPD.

Este módulo fornece a lógica principal para:
- Coleta de casos do Portal Público do CNPD
- Download e armazenamento de imagens oficiais
...
Uso:
    ...
    result = synchronize_cnpd(max_pages=100, download_images=True)
"""
...
def save_raw_page(coleta_id: int, page: int, payload: dict[str, Any]) -> Path:
    """
    Salva uma página de resposta da API como JSON raw.

    Útil para auditoria e reprocessamento posterior.

    Args:
        coleta_id: ID da coleta à qual a página pertence.
        page: Número da página.
        payload: Dados da resposta da API.
...
def create_collection(conn: sqlite3.Connection, order: str, start_page: int) -> int:
    """
    Cria um registro de coleta no banco e retorna o ID gerado.

    Args:
        conn: Conexão SQLite ativa.
        order: Ordem de classificação usada.
        start_page: Página inicial da coleta.

    Returns:
        int: ID da coleta criada.
    """
    cursor = conn.execute(
        """
...
def finalize_collection(
    ...
    *,
    ...
    """
    Finaliza um registro de coleta com status e estatísticas.

    Args:
        conn: Conexão SQLite ativa.
        collection_id: ID da coleta a finalizar.
        status: Status final ('COMPLETA' ou 'FALHOU').
        pages_read: Número de páginas lidas.
        records_read: Número de registros processados.
...
def upsert_case(conn: sqlite3.Connection, case: dict[str, Any]) -> None:
    """
    Insere ou atualiza um caso no banco.

    Usa INSERT ... ON CONFLICT para upsert baseado no cnpd_id.

    Args:
        conn: Conexão SQLite ativa.
        case: Dados do caso normalizado.
    """
    now = utc_now()
    conn.execute(
        """
...
def mark_case_seen(conn: sqlite3.Connection, collection_id: int, cnpd_id: int) -> None:
    """
    Marca um caso como encontrado durante a coleta.

    Usado para rastrear quais casos foram encontrados em cada coleta.

    Args:
        conn: Conexão SQLite ativa.
        collection_id: ID da coleta atual.
        cnpd_id: ID do caso.
    """
    conn.execute(
        """
...
def has_official_cnpd_image(
    conn: sqlite3.Connection,
    cnpd_id: int,
) -> bool:
    """
    Verifica se o caso já possui uma imagem oficial do CNPD.
    ...
    Returns:
        bool: True se já houver imagem oficial.
    """
    row = conn.execute(
        """
        SELECT 1
...
def deduplicate_cnpd_media(
    conn: sqlite3.Connection,
) -> int:
    """
    Remove mídias duplicadas oficiais do CNPD.

    Preserva a primeira mídia oficial de cada combinação cnpd_id + sha256.
    Uploads manuais (origem = INVESTIGADOR) não são afetados.
...
def store_source_image(conn: sqlite3.Connection, image: dict[str, Any]) -> None:
    """
    Armazena metadados de uma imagem baixada da fonte CNPD.

    Args:
        conn: Conexão SQLite ativa.
        image: Dicionário com metadados da imagem.
    """
...
def synchronize_cnpd(
    *,
    ...
    """
    Sincroniza casos do CNPD com o banco local.

    Realiza a coleta de casos do portal público, processa cada registro,
    salva no banco SQLite e opcionalmente baixa imagens oficiais.

    Args:
        max_pages: Máximo de páginas a ler.
        start_page: Página inicial para começar.
        order: Ordem de classificação ("MAIS_RECENTE" ou "MAIS_ANTIGO").
        page_delay: Delay entre requisições (segundos).
        download_images: Se True, baixa imagens dos casos.
        image_delay: Delay entre downloads de imagem (segundos).

    Returns:
        dict: Resultado da coleta com:
            - collection_id: ID da coleta
            - status: "COMPLETA" ou "FALHOU"
            - pages_read, records_read, images_downloaded, image_errors

    Raises:
        ValueError: Se max_pages < 1.
    """
    if max_pages < 1:
    ...
    try:
        for page, payload in client.iter_pages(
            ...
            with get_connection() as conn:
                ...
                for raw in records:
                    # Ignora casos de menores por política de privacidade
                    if _is_minor_from_source(raw):
                        logger.info(
                            "Registro CNPD ignorado por política de proteção de menores"
                        )
                        continue
                    ...
                    # Download de imagem se solicitado e não existe
                    if download_images and not has_official_cnpd_image(
                        conn,
                        case["cnpd_id"],
                    ):
                        try:
        ...
        # Remoção de duplicatas ao final da coleta
        with get_connection() as conn:
            duplicates_removed = deduplicate_cnpd_media(conn)

            if duplicates_removed:
                logger.info(
                    "Mídias CNPD duplicadas removidas ao fim da coleta: %s",
                    duplicates_removed,
                )
...
def _is_minor_from_source(record: dict[str, Any]) -> bool:
    """
    Determina se o registro representa um menor de idade.

    Verifica vários campos possíveis de idade na fonte. Casos com
    menores de 18 anos são excluídos por políticas de privacidade.

    Args:
        record: Registro bruto da API CNPD.

    Returns:
        bool: True se o registro indica menor de idade.
    """
    possible_keys = (
        "idadeAtual",
        "idade_atual",
        "idade",
        "idadeDesaparecimento",
        "idadeNoDesaparecimento",
